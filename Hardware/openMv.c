...
int distance;
int flag;
int flag_total=0,next_angle,angle_temp;
extern int angle,last_angle;
				...
				if(RxFlag1)
					{
						...
						OLED_ShowString(2,1,"Theta:");
		                OLED_ShowSignedNum(2, 6, theta, 3);
                        angle_temp = theta;
						angle = (int) angle_temp * 0.6 + last_angle * 0.25 + next_angle * 0.15; //回传数据
						next_angle = last_angle;
						last_angle = angle_temp;   
						//angle<0  左转  反之右转
						//if((theta>20)||(theta<-40))//误差过大的范围，误差视为上一次合理的误差
						//{
						//	angle = last_angle;
						//}
						//else if((theta<10)&&(theta>-10))//直道行驶调整角度
						//{
						//	angle=theta/2;
						//}
						//else angle=theta;//过弯可能出现的角度误差
						//last_angle=angle;
